"""Freetype via SourceForge (savannah.gnu.org suele devolver HTTP 502 en CI)."""
...
class FreetypeRecipe(Recipe):
    # 2.13.2: espejo SourceForge estable (evita 502 de savannah)
    version = "2.13.2"
    url = (
    ...
    def get_recipe_env(self, arch=None, with_harfbuzz=False):
        ...
        if with_harfbuzz:
            harfbuzz_build = self.get_recipe("harfbuzz", self.ctx).get_build_dir(arch.arch)
